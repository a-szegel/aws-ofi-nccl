...
**Flow:**
1. Test sets expectation: `EXPECT_CALL(*mock, fi_send(...))`
2. Test calls: `fi_send(...)`
3. Linker redirects to: `__wrap_fi_send(...)`
4. Wrapper calls: `g_libfabric_mock->fi_send(...)`
...
**Cause:** Test calling inline function with NULL struct pointers.

**Example:**
```cpp
struct fid_ep ep = {};  // ep.msg is NULL
fi_send(&ep, ...);      // Crashes: ep->msg->send(...)
```

**Fix:** Don't call inline functions directly in tests. Test higher-level aws-ofi-nccl functions that use libfabric, not libfabric functions themselves.
...
✅ Test aws-ofi-nccl functions that use libfabric, not libfabric itself
✅ Use `NiceMock` to avoid warnings for uninteresting calls
✅ Set up expectations before calling code under test
✅ Verify both success and error paths
✅ Use meaningful test names: `TEST_F(OfiUtils, GetinfoHandlesNoProviders)`
✅ Clean up in `TearDown()` - delete mock and clear global pointer

### DON'T

❌ Don't call inline libfabric functions directly with NULL structs
❌ Don't forget to add new functions to `--wrap` flags
❌ Don't mix inline and non-inline function definitions
❌ Don't use `StrictMock` unless you want to verify every single call
❌ Don't forget to rebuild after modifying mock files
...
2. Review example tests in `libfabric_mock_test.cpp`
3. Open an issue on the aws-ofi-nccl GitHub repository
